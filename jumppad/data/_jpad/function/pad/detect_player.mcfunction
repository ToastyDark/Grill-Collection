# Remove Tag?
$execute align xz positioned ~1 ~ ~1 as @a[tag=jpad_over] unless entity @s[dx=$(dlength),dy=1,dz=$(dwidth)] run function _jpad:pad/left_pad


# Check for players who are within the pad
$execute align xz positioned ~1 ~ ~1 as @a if entity @s[dx=$(dlength),dy=4,dz=$(dwidth)] at @s run tag @s add jpad_over
